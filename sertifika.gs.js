...
/**
 * Seçilen konu başlıklarına göre içerikleri getirir. (İyileştirilmiş versiyon)
 */
...
function getHazardYearsForFirm(firmName) {
  ...
  if (!flistSheet) {
    ...
    return 1; // Varsayılan olarak Çok Tehlikeli
  }
  ...
  const firmMatch = flistData.find(row => normalizeString(row[0]) === normalizeString(firmName));
  if (firmMatch) {
...
function processFormWithRowNumbers(rowNumbersStr, templates, firm, date1Str, date3Str, date5Str, trainingHours1, trainingHours2, trainingHours3, selectedTopicNames) {
  ...
  function fmt(d){ return d ? formatDateToDDMMYYYY(d) : '';
  }
...
// ---- Katılımcı Listesi (şablon 5) ----
  ...
  if (templates.includes(KATILAN_LISTESI_INDEX)) {
    try {
      ...
      const newFileName = `${firm.substring(0,15)} - Katılımcı Listesi${tarihlerStr ?
      ' - ' + tarihlerStr : ''}`;
  ...
  selectedPersonnel.forEach(person => {
    ...
    otherTemplates.forEach(templateIndex => {
      try {
        ...
        for (let k in data) body.replaceText(escapeRegExp(`{{${k}}}`), data[k] ||
        '');
